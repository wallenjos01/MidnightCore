...
public class AdapterImpl implements Adapter {
    ...
    public void sendTitle(Player player, Component component) {
        EntityPlayer ep = ((CraftPlayer) player).getHandle();
        IChatBaseComponent bc = convert(component);
        if(bc != null) ep.f.a(new ClientboundSetTitleTextPacket(bc));
    }
    ...
    public void sendSubtitle(Player player, Component component) {
        EntityPlayer ep = ((CraftPlayer) player).getHandle();
        IChatBaseComponent bc = convert(component);
        if(bc != null) ep.f.a(new ClientboundSetSubtitleTextPacket(bc));
    }

    @Override
    public void setTitleAnimation(Player player, int i, int i1, int i2) {
        EntityPlayer ep = ((CraftPlayer) player).getHandle();
        ep.f.a(new ClientboundSetTitlesAnimationPacket(i, i1, i2));
    }

    @Override
    public void clearTitles(Player player) {
        EntityPlayer ep = ((CraftPlayer) player).getHandle();
        ep.f.a(new ClientboundClearTitlesPacket(false));
    }

    @Override
    public void resetTitles(Player player) {
        EntityPlayer ep = ((CraftPlayer) player).getHandle();
        ep.f.a(new ClientboundClearTitlesPacket(true));
    }
    ...
    public void loadTag(Player player, ConfigSection configSection) {
        ...
        for (MobEffect mobeffect : epl.eB()) {
            epl.f.a(new PacketPlayOutEntityEffect(epl.ar(), mobeffect, false));
        }
    }
    ...
    public Identifier getItemId(ItemStack is) {
        ...
        return Identifier.parse(Objects.requireNonNull(BuiltInRegistries.g.b(reflector.getHandle(is).h())).toString());
    }
    ...
    public @Nullable ConfigObject saveComponent(ItemStack is, Identifier component) {
        ...
        DataComponentType<?> type = BuiltInRegistries.ao.a(MinecraftKey.a(component.getNamespace(), component.getPath())); // DATA_COMPONENT_TYPE, get
        if(type == null) {
    ...
    public Stream<Identifier> getComponentIds(ItemStack is) {
        ...
        return item.a().c().map(typed -> {
            ...
            MinecraftKey key = BuiltInRegistries.ao.b(typed.a()); // DATA_COMPONENT_TYPE, getKey
    ...
    public org.wallentines.mcore.ItemStack.ComponentPatchSet getComponentPatch(ItemStack is) {
        ...
        DataComponentPatch.c res = item.e().e(); // DataComponentPatch, split()
        for(DataComponentType<?> type : res.a().b()) {

            MinecraftKey id = BuiltInRegistries.ao.b(type);
            if(id == null) {
    ...
    public void kickPlayer(Player player, Component message) {
        ((CraftPlayer) player).getHandle().f.a(convert(message));
    }

    @Override
    public Color getRarityColor(ItemStack itemStack) {
        Integer clr = reflector.getHandle(itemStack).C().a().f();
