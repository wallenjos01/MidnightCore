...
public class SkinUpdaterImpl implements SkinUpdater {
    ...
    public void changePlayerSkin(Player player, @Nullable Skin skin) {
        ...
        PacketPlayOutExperience exp = new PacketPlayOutExperience(epl.cq, epl.co, epl.cp);
        ...
        if (!observers.isEmpty()) {
            ...
            List<DataWatcher.c<?>> entityData = epl.ap().c(); // DataValue<?>, getEntityData(), getNonDefaultValues()
            if (entityData != null) {
        ...
        for (MobEffect mobeffect : epl.ex()) {
            epl.c.a(new PacketPlayOutEntityEffect(epl.al(), mobeffect, true));
        }
    }
}
