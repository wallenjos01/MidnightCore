...
public class MainCommand {
    ...
    private static int emptyCommand(CommandContext<CommandSourceStack> context) {
        ...
        try {
            ...
            context.getSource().sendSuccess(ConversionUtil.toComponent(prov.getMessage(Constants.makeNode("command.main"), "en_us")), false);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
        return 0;
    ...
    private static int modulesCommand(CommandContext<CommandSourceStack> context) {
        ...
        try {
            ...
            context.getSource().sendSuccess(ConversionUtil.toComponent(prov.getMessage(Constants.makeNode("command.modules.header"), "en_us")), false);
            ...
            for(ModuleInfo<MidnightCoreAPI> info : Registries.MODULE_REGISTRY) {
                ...
                if(manager.isModuleLoaded(info.getId())) {
                    context.getSource().sendSuccess(ConversionUtil.toComponent(prov.getMessage(Constants.makeNode("command.modules.enabled"), "en_us", cp)), false);
                } else {
                    context.getSource().sendSuccess(ConversionUtil.toComponent(prov.getMessage(Constants.makeNode("command.modules.disabled"), "en_us", cp)), false);
                }
            }
        } catch (Exception ex) {
            ex.printStackTrace();
